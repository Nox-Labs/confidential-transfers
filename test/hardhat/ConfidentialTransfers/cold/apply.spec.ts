...
describe("ConfidentialTransfers/cold", function () {
  ...
  describe("cApply()", function () {
    ...
    beforeEach(async function () {
      proof = await f.cApply("cold", f.user2, [0])
    })
    ...
    it("Should remove the pending transfer from the recipient's account", async function () {
      const accountAfter = await f.token.getAccount(f.user2.address)
      expect(accountAfter.pendingTransfers.length).to.equal(0)
    })
  })

  describe("cApply() with multiple pending transfers", function () {
    beforeEach(async function () {
      await f.cTransfer("cold", f.user1, f.user2.address, f.TRANSFER_AMOUNT)
      await f.cTransfer("cold", f.user1, f.user2.address, f.TRANSFER_AMOUNT)
    })
  ...
  describe("cApply() reverted", function () {
