...
describe("ConfidentialTransfers", function () {
  describe("Cold:ConfidentialTransfers", function () {
    describe("Cold:ConfidentialTransfers:cBurn()", function () {
      ...
      beforeEach(async function () {
        ...
        TARGET_AMOUNT_AFTER_MINT = f.DEPOSIT_AMOUNT - f.BURN_AMOUNT
      })

      describe("State Changes", function () {
        describe("Payload", function () {
          beforeEach(async function () {
            await f.cBurn("cold", f.user1, f.BURN_AMOUNT)
          })
        ...
        it("Should update auditor reports", async function () {
          ...
          const params = f.sdk.getBurnParams(proof, auditorReports)
          await f.token.connect(f.user1).cBurn(params)
          const accountAfter = await f.token.getAccount(f.user1.address)
        ...
        it("Should emit event", async function () {
          ...
          const proof = f.getProofOutput(proofFilename)
          const params = f.sdk.getBurnParams(proof)
          await expect(f.token.connect(f.user1).cBurn(params)).to.emit(
            f.token,
            "CBurned"
          )
        })
      ...
      describe("Reverts", function () {
        it("Should revert if the account is not initialized", async function () {
          const params = f.sdk.getBurnParams(f.MOCK_PROOF_OUTPUT)
          await expect(
            f.token.connect(f.userUninitialized).cBurn(params)
        ...
        it("Should revert if the proof verification fails", async function () {
          ...
          const params = f.sdk.getBurnParams(proof)
          await expect(
            f.token.connect(f.user1).cBurn(params)
