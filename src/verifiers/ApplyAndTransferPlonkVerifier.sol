...
contract PlonkVerifier {
    ...
    uint256 constant Qmx = 11795123554743814186836695126243294668091077337287059208724122743293400438184;
    uint256 constant Qmy = 19004612608481914308282183807055748214504705533106885329637916172774730810713;
    uint256 constant Qlx = 2528069538610530307895693168141807499662600525095309153531312702895734216079;
    uint256 constant Qly = 17562474433156706124189910666112391595344267417447626899523470227939516181447;
    uint256 constant Qrx = 2057318968426706362912966072086073698096637655009502536897337136349979869040;
    uint256 constant Qry = 13512599214900555237313439934113427515521562766263324504214005105677486342458;
    uint256 constant Qox = 8739583337698348176230886424934373785114052281338498074284185418426675418218;
    uint256 constant Qoy = 8791275020554366809549240376297482338874533207230604106327045805769914345562;
    uint256 constant Qcx = 16026868180198448610054553246276429882920232925872842585039510907608737629478;
    uint256 constant Qcy = 162545838602773517106188348746039006693274378588251324210542104496158878304;
    uint256 constant S1x = 2059552738222997707790155484332203689871586492492461525199623468523397525763;
    uint256 constant S1y = 21228684597607960306532221121679030994809946449547001881938777142545645356589;
    uint256 constant S2x = 21135016616642084752362227807078234413412758162706084083192595223117041122817;
    uint256 constant S2y = 9213950171693567762357424148690554919367957057153537167810998338132906639678;
    uint256 constant S3x = 15150967852164075475321686892431852821911649409773961553360408200264981274228;
    uint256 constant S3y = 3420188928031025475458859523718510424682199580200112705933369835120103970192;
    ...
    uint16 constant lastMem = 1536;

    function verifyProof(uint256[24] calldata, uint256[23] calldata _pubSignals) public view returns (bool) {
        assembly {
            /////////
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 1248), calldataload(pA))
                mstore(add(mIn, 1280), calldataload(add(pA, 32)))
                mstore(add(mIn, 1312), calldataload(pB))
                mstore(add(mIn, 1344), calldataload(add(pB, 32)))
                mstore(add(mIn, 1376), calldataload(pC))
                mstore(add(mIn, 1408), calldataload(add(pC, 32)))

                beta := mod(keccak256(mIn, 1440), q)
            ...
            function calculateLagrange(pMem) {
                ...
                mstore(add(pMem, pEval_l6), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l7), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l8), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l9), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l10), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l11), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l12), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l13), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l14), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l15), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l16), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l17), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l18), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l19), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l20), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l21), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l22), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))

                w := mulmod(w, w1, q)

                mstore(add(pMem, pEval_l23), mulmod(n, mod(add(sub(mload(add(pMem, pXi)), w), q), q), q))
            ...
            function calculatePI(pMem, pPub) {
                ...
                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l6)), calldataload(add(pPub, 160)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l7)), calldataload(add(pPub, 192)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l8)), calldataload(add(pPub, 224)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l9)), calldataload(add(pPub, 256)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l10)), calldataload(add(pPub, 288)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l11)), calldataload(add(pPub, 320)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l12)), calldataload(add(pPub, 352)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l13)), calldataload(add(pPub, 384)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l14)), calldataload(add(pPub, 416)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l15)), calldataload(add(pPub, 448)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l16)), calldataload(add(pPub, 480)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l17)), calldataload(add(pPub, 512)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l18)), calldataload(add(pPub, 544)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l19)), calldataload(add(pPub, 576)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l20)), calldataload(add(pPub, 608)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l21)), calldataload(add(pPub, 640)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l22)), calldataload(add(pPub, 672)), q)), q), q)

                pl := mod(add(sub(pl, mulmod(mload(add(pMem, pEval_l23)), calldataload(add(pPub, 704)), q)), q), q)
